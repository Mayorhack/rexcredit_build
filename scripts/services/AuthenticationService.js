  ...
  mifosX.services = _.extend(module, {
    AuthenticationService: function (
      ...
      var onLoginSuccess = function (response) {
        ...
        if (
          data.isTwoFactorAuthenticationRequired != null &&
          data.isTwoFactorAuthenticationRequired == true
        ) {
      ...
      var getUserDetails = function (response) {
        ...
        httpService
          .get(apiVer + "/userdetails?access_token=" + data.access_token)
          .then(onLoginSuccess)
      ...
      var getAccessToken = function () {
        var refreshToken =
          localStorageService.getFromLocalStorage("tokendetails").refresh_token;
        httpService.cancelAuthorization();
        httpService
          .post(
            "/fineract-provider/api/oauth/token?&client_id=community-app&grant_type=refresh_token&client_secret=123&refresh_token=" +
              refreshToken
          )
      ...
      this.authenticateWithUsernamePassword = function (credentials) {
        ...
        if (SECURITY === "oauth") {
          httpService
            .post(
              "/fineract-provider/api/oauth/token?username=" +
      ...
      var getTokenFromStorage = function (user) {
        var twoFactorStorage =
          localStorageService.getFromLocalStorage("twofactor");
      ...
      this.validateOTP = function (token, rememberMe) {
        ...
        httpService
          .post(apiVer + "/twofactor/validate?token=" + token)
      ...
      scope.$on("OnUserPreLogout", function (event) {
        ...
        httpService.post(
          apiVer + "/twofactor/invalidate",
          '{"token": "' + twoFactorAccessToken + '"}'
        );
      });
    },
  });
